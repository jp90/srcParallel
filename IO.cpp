...
void IO::writeVTKMasterfile(const 	MultiIndexType & griddimension,
	...
	// 2x1-Gitter
	RealType localgriddimension[2];
	localgriddimension[0] = griddimension[0]/2.0;
	localgriddimension[1] = griddimension[1];

	// als Argumente uebergeben ?!
	RealType stencilwidth = 3;
	RealType processorgridcoords[2];
	// processorgridcoords??
	...
	RealType x1 = processorgridcoords[0] * (localgriddimension[0]) - stencilwidth;
	RealType x2 = (processorgridcoords[0] + 1) * (localgriddimension[0]) + stencilwidth - 1;
	RealType x3 = processorgridcoords[1] * (localgriddimension[1]) - stencilwidth;
	RealType x4 = (processorgridcoords[1] + 1) * (localgriddimension[1]) + stencilwidth - 1;
...
void IO::writeVTKSlavefile(const 	MultiIndexType & griddimension,
	...
	RealType stencilwidth = 3;
	RealType processorgridcoords[2];
	//processorgridcoords?? --> if processor 0 --> linkes Teilgebiet, if processor 1 --> rechtes Teilgebiet ?? => allg.?
	processorgridcoords[0] = 0;
	processorgridcoords[1] = 1;
	...
	RealType x5 = processorgridcoords[0] * (localgriddimension[0]) - stencilwidth;
	RealType x6 = processorgridcoords[1] * (localgriddimension[1]) - stencilwidth;
	RealType x7 = (processorgridcoords[d] + 1) * (localgriddimension[d]) + stencilwidth;
	RealType x8 = (processorgridcoords[d] + 1) * (localgriddimension[d]) + stencilwidth;
